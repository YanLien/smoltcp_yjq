...
impl BridgeWrapper {
    ...
    pub fn add_port(&self, port_config: Config, port_device: BridgeDevice, port_num: u8, port_now: Instant) -> Result<(), &'static str> {
    ...
    pub fn receive_frame(&self, time: Instant) -> Option<(u8, Vec<u8>)> {
        ...
        for (port_num, port) in bridge.ports.iter_mut() {
            if let Some((rx_token, _)) = port.port_device.lock().receive(time) {
                let mut frame_data = None;
                rx_token.consume(&mut |buffer: &mut [u8]| {
                    let frame_buffer = buffer.to_vec();
                    match EthernetFrame::new_checked(&frame_buffer) {
                        Ok(_) => {
                            debug!("Received valid frame on port {}", port_num);
                            frame_data = Some(frame_buffer);
                        }
                        Err(_) => {
                            debug!("Received invalid frame on port {}", port_num);
                        }
                    }
                });
